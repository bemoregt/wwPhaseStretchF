...
void ofApp::setup(){
    ...
    before.load("/Users/kerbal/Desktop/lena.jpg");
    cv::Mat src= toCv(before);
    ...
    cv::Mat pst, pstKernel;
    PST(src, handles, true, pst , pstKernel);
    cv::Mat pst2;
    cv::normalize(pst, pst2, 255, 0, cv::NORM_MINMAX, CV_8U);
    toOf(pst2, after);
    after.update();
}
